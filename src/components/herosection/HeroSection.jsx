...
export default function HeroSection({ onOpenGetStarted, setActivePage }) {
  ...
  return (
    <section className="relative w-full h-screen min-h-[620px] max-h-[1080px] overflow-hidden bg-white text-slate-900 select-none">
      {/* Blueprint dot-grid ambient backdrop (concentrated behind the text panel, fading toward the video) */}
      ...
      <div className="relative z-10 w-full h-full flex flex-col pt-20 sm:pt-24 lg:pt-28 lg:flex-row">

        {/* Text Panel */}
        <motion.div
          className={`relative z-20 order-2 lg:order-none w-full lg:w-[40%] xl:w-[37%] shrink-0 flex flex-col justify-center gap-5 px-5 sm:px-10 lg:px-12 xl:px-16 py-8 lg:py-0 pointer-events-auto ${isRTL ? 'text-right items-end' : 'text-left items-start'}`}
        ...
        >
          ...
          <h1 className="font-headline-xl text-3xl sm:text-4xl md:text-4xl lg:text-[40px] xl:text-[46px] font-black leading-snug sm:leading-tight tracking-tight py-1 glass-text-shine">
            {t('hero.titlePrefix')}{' '}
            <span>
          ...
          {/* Capability strip — compact, ruled, no card chrome */}
          <div className="hidden lg:flex items-stretch gap-5 pt-6 mt-2 border-t border-slate-200 w-full">
            {[
              ...
              <div key={idx} className={`flex-1 flex flex-col gap-1.5 ${idx !== 0 ? 'border-s border-slate-200 ps-5' : ''}`}>
                <Icon className="w-4 h-4 text-sky-600" />
                <h4 className="text-[11px] font-bold text-slate-900 leading-tight font-label-md">{title}</h4>
                <p className="text-[10.5px] text-slate-500 leading-snug">{sub}</p>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        {/* Video Panel — full-bleed schematic viewport */}
        <motion.div
          className="relative flex-1 min-h-[42vh] lg:min-h-0 lg:h-full order-1 lg:order-none overflow-hidden bg-gradient-to-br from-sky-50/70 via-white to-white pointer-events-auto"
        ...
        >
          <div className="absolute inset-3 sm:inset-4 lg:inset-6">
            ...
            <div className="absolute top-0 left-0 w-5 h-5 border-t-2 border-l-2 border-sky-400/60 pointer-events-none" />
            <div className="absolute top-0 right-0 w-5 h-5 border-t-2 border-r-2 border-sky-400/60 pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-5 h-5 border-b-2 border-l-2 border-sky-400/60 pointer-events-none" />
            <div className="absolute bottom-0 right-0 w-5 h-5 border-b-2 border-r-2 border-sky-400/60 pointer-events-none" />
            ...
            {/* Live status badge */}
            <div className={`absolute top-3 ${isRTL ? 'right-3' : 'left-3'} inline-flex items-center gap-1.5 bg-white/90 backdrop-blur-md border border-slate-200 rounded-full px-2.5 py-1 shadow-sm z-30`}>
              <span className="relative flex h-1.5 w-1.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75" />
                <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-sky-500" />
              </span>
              <span className="text-[10px] font-bold tracking-[0.15em] text-slate-700 font-label-md">LIVE</span>
            </div>
          </div>

          {/* Interactive Controls */}
          <div className={`absolute bottom-4 sm:bottom-6 ${isRTL ? 'left-4 sm:left-6' : 'right-4 sm:right-6'} z-30 flex items-center gap-3`}>
            <button
              onClick={togglePlayPause}
              title={isPlaying ? "Pause Video" : "Play Video"}
              className="p-3 rounded-full bg-white/90 hover:bg-sky-500 hover:text-white backdrop-blur-lg border border-slate-300 text-slate-800 transition-all shadow-md hover:scale-105 active:scale-95 cursor-pointer group"
            >
              {isPlaying ? (
            ...
            <button
              ...
              className={`p-3 rounded-full backdrop-blur-lg border transition-all shadow-md hover:scale-105 active:scale-95 cursor-pointer ${isMuted
