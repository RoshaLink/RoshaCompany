...
export function Awards({
  ...
  if (variant === "stamp") {
    ...
    return (
      <div
        className={cn(
          "award-stamp-container relative mx-auto flex h-52 w-52 items-center justify-center",
          className
        )}
      >
        <svg
        ...
        >
          <defs>
            <path
              id="top-curve"
              d={createTextPath(52, "top-curve")}
              fill="none"
            />
            <path
              id="bottom-curve"
              d={createTextPath(58, "bottom-curve")}
              fill="none"
          ...
          <path
            ...
            strokeWidth="0.8"
            className="award-stamp-serrated"
          />
          ...
          <circle
            ...
            cy="96"
            r="74"
            className="award-stamp-inner-circle"
            strokeWidth="0.8"
          />

          {/* Curved text - top */}
          <text className="award-stamp-curved-top text-xs font-bold uppercase tracking-wider">
            <textPath
              href="#top-curve"
              startOffset="50%"
              textAnchor="middle"
          ...
          {/* Curved text - bottom */}
          <text className="award-stamp-curved-bottom text-[9px] font-semibold tracking-wider">
            <textPath
              href="#bottom-curve"
              startOffset="50%"
              textAnchor="middle"
        ...
        <div className="relative z-10 text-center px-4">
          ...
          {recipient && (
            <div className="award-stamp-recipient text-xs font-bold mt-1">{recipient}</div>
          )}
          {date && <div className="award-stamp-date text-[10px] italic opacity-75">{date}</div>}
        </div>
      </div>
    );
  }

  // Award Variant (Laurel Wreath)
  if (variant === "award") {
    const LaurelWreath = () => (
      <svg
        className="award-laurel-svg absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] h-[90%] pointer-events-none opacity-20 dark:opacity-30"
    ...
    return (
      <div
        className={cn(
          "award-card-wrapper award-variant-wreath relative flex flex-col items-center justify-center p-6 text-center overflow-hidden rounded-2xl",
          className
        )}
      >
        <LaurelWreath />
        <div className="relative z-10 px-4 flex flex-col items-center">
          ...
          <div className="award-decorative-line mx-auto my-2 h-[2px] w-24 rounded-full bg-sky-500/50" />

          <p className="award-subtitle text-sm font-medium mb-3 max-w-[260px] opacity-85">
            {subtitle}
          </p>

          {recipient && (
            <p className="award-recipient text-xs font-semibold text-sky-600 dark:text-sky-400 italic mb-1">
              {recipient}
            </p>
          )}

          {date && (
            <div className="award-date text-xs font-bold opacity-70 mt-1">
              {date}
            </div>
          )}
        </div>
  ...
  if (variant === "certificate") {
    const BadgeIcon = () => (
      <svg
        className="award-certificate-ribbon w-8 h-8 mx-auto mb-2 text-sky-500 fill-sky-500"
        viewBox="0 0 24 24"
      >
    ...
    return (
      <div
        className={cn(
          "award-card-wrapper award-variant-certificate relative flex flex-col items-center justify-center rounded-2xl p-3",
          className
        )}
      >
        <div className="award-cert-inner w-full rounded-xl p-5 text-center flex flex-col items-center justify-between h-full">
          <BadgeIcon />
          
          <h4 className="award-cert-heading text-xs font-mono uppercase tracking-widest text-sky-500 font-bold mb-1">
            Certificate of Excellence
          </h4>

          <h3 className="award-title text-xl font-bold tracking-tight mb-2">
            {title}
          </h3>

          <p className="award-cert-certify text-[11px] opacity-70 mb-1">
            Awarded to
          </p>

          <div className="award-recipient text-base font-bold text-sky-600 dark:text-sky-400 border-b border-sky-500/30 pb-1 mb-2">
            {recipient}
          </div>

          <p className="award-subtitle text-xs opacity-80 mb-3 max-w-[240px]">
            {subtitle}
          </p>

          <div className="flex items-center justify-center gap-2 mt-auto text-xs opacity-75">
  ...
  // Badge Variant (Embossed Seal)
  if (variant === "badge") {
    const BadgeSeal = () => (
      <svg
        className="award-badge-seal-svg w-16 h-16 shrink-0 fill-sky-500 text-sky-500"
    ...
    return (
      <div
        className={cn(
          "award-card-wrapper award-variant-badge relative flex items-center p-5 rounded-2xl",
          className
        )}
      >
        <div className="flex items-center gap-4 w-full">
          <BadgeSeal />
          <div className="award-badge-content border-l border-slate-200 dark:border-slate-800 pl-4">
            <h3 className="award-title text-xl font-bold tracking-tight mb-1">
              {title}
            </h3>
            <p className="award-subtitle text-xs opacity-80 mb-2">
              {subtitle}
            </p>
            <div className="flex items-center gap-3 text-[11px] opacity-75 font-mono">
  ...
  // ID-Card / Pass Variant
  if (variant === "id-card") {
    return (
      <div
        className={cn(
          "award-card-wrapper award-variant-idcard relative flex flex-col justify-between p-6 rounded-2xl overflow-hidden",
          className
        )}
      >
        <div className="flex justify-between items-start mb-4">
          <div className="award-id-tag inline-block rounded-md px-3 py-1 text-[11px] font-mono font-bold uppercase">
            {date || "2025 HONORS"}
          </div>
        ...
        <div className="my-auto">
          <h3 className="award-title text-2xl font-black tracking-tight mb-1">
            {title}
          </h3>
          <p className="award-subtitle text-xs font-light tracking-wide opacity-80">
            {subtitle}
          </p>
        </div>

        <div className="border-t border-slate-200/50 dark:border-slate-800/50 pt-3 mt-4 flex justify-between items-center">
          <div>
            <div className="text-[10px] font-mono uppercase opacity-60">Verified</div>
            <div className="award-recipient text-sm font-bold text-sky-600 dark:text-sky-400">
              {recipient}
            </div>
          </div>
